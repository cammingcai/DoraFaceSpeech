...
//        }
        ...
        mFaceModel = mFaceModule.detect(imageData, mPreviewRect.height(), mPreviewRect.width());
        processUIStrategy(new UILivenessResultRunnable(mFaceModel));
    }
...
//                    break;
    ...
    private void saveLivenessImage(final LivenessTypeEnum type, final int[] argbByte,
        ...
        if (!mBase64ImageMap.containsKey(type.name())) {
            Bitmap image = BitmapUtils.createLivenessBitmap( argbByte, roundRect,false);
    ...
    private void processUICallback(FaceStatusEnum status) {
        ...
        if (status == FaceStatusEnum.OK
            ...
            if (mILivenessStrategyCallback != null) {
                ...
                if(mFaceModel==null)
                    mILivenessStrategyCallback.onLivenessCompletion(status, getStatusTextResId(status), null,null);
                else
                    mILivenessStrategyCallback.onLivenessCompletion(status, getStatusTextResId(status), null,mFaceModel.getArgbImage());
               // mILivenessStrategyCallback.onLivenessCompletion(status, getStatusTextResId(status), mBase64ImageMap,mImageData);
            }
        } else {
//            Log.e(TAG, "processUICallback " + status.name());
            if (mILivenessStrategyCallback != null) {
                if(mFaceModel==null)
                    ...
                    mILivenessStrategyCallback.onLivenessCompletion(status, getStatusTextResId(status), null,mFaceModel.getArgbImage());
              //  mILivenessStrategyCallback.onLivenessCompletion(status, getStatusTextResId(status), null,mImageData);
            }
        }
    }
